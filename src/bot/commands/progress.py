...
PROGRESS_FILE = BASE_DIR / "docs" / "reading_in_progress.md"
            ...
            if 0 <= val <= 100:
                if val == 100:
                    ...
                    await update.message.reply_text(
                        "✅ Grattis! Boken är avslutad. Glöm inte att logga den på Goodreads!"
                    )
                    return
...
async def start_reading(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not validation.get("valid", False):
        ...
        await update.message.reply_text(
            f"❌ Kunde inte identifiera boken: {reason}\n\nFörsök igen med /read Titel - Författare"
        )
        return
    ...
    save_pending_confirmation(
        chat_id,
        {"raw_input": input_text, "corrected": {"title": title, "author": author}},
    )
